...
const App = () => {
  ...
  const onMint = async () => {
    ...
    if(parseInt(numberOfWallet) + counter > 20) {
    ...
    try {
      let tx = await contract.mintToken(counter, { value: BigNumber.from(1e9).mul(BigNumber.from(1e9)).mul(5).div(100).mul(counter), from: walletAddress})
